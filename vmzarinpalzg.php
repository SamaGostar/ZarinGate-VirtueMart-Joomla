...
class plgVmPaymentVmzarinpalzg extends vmPSPlugin {
	...
	function getTableSQLFields() {
	    ...
	    'Vmzarinpalzg_custom' => ' varchar(255)  ',
	    'Vmzarinpalzg_response_mc_gross' => ' decimal(10,2) DEFAULT NULL ',
	    'Vmzarinpalzg_response_mc_currency' => ' char(10) DEFAULT NULL',
	    'Vmzarinpalzg_response_invoice' => ' char(32) DEFAULT NULL',
	    'Vmzarinpalzg_response_protection_eligibility' => ' char(128) DEFAULT NULL',
	    'Vmzarinpalzg_response_payer_id' => ' char(13) DEFAULT NULL',
	    'Vmzarinpalzg_response_tax' => ' decimal(10,2) DEFAULT NULL ',
	    'Vmzarinpalzg_response_payment_date' => ' char(28) DEFAULT NULL',
	    'Vmzarinpalzg_response_payment_status' => ' char(50) DEFAULT NULL',
	    'Vmzarinpalzg_response_mc_fee' => ' decimal(10,2) DEFAULT NULL ',
	    'Vmzarinpalzg_response_payer_email' => ' char(128) DEFAULT NULL',
	    'Vmzarinpalzg_response_last_name' => ' char(64) DEFAULT NULL',
	    'Vmzarinpalzg_response_first_name' => ' char(64) DEFAULT NULL',
	    'Vmzarinpalzg_response_business' => '  char(128) DEFAULT NULL',
	    'Vmzarinpalzg_response_receiver_email' => '  char(128) DEFAULT NULL',
	    'Vmzarinpalzg_response_transaction_subject' => ' char(128) DEFAULT NULL',
	    'Vmzarinpalzg_response_residence_country' => ' char(2) DEFAULT NULL',
	    'Vmzarinpalzgresponse_raw' => ' char DEFAULT NULL'
		);
		return $SQLfields;
	}
...
$mm=$dbcccow->email;							
	...
	function plgVmOnPaymentNotification() {
		...
		if (!class_exists('VirtueMartModelOrders'))
		require( JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'orders.php' );
		$Vmzarinpalzg_data = JRequest::get('post');
		if (!isset($Vmzarinpalzg_data['on'])) {
			return;
		}
		$order_number = $Vmzarinpalzg_data['on'];
		$virtuemart_order_id = VirtueMartModelOrders::getOrderIdByOrderNumber($Vmzarinpalzg_data['invoice']);
		...
		$this->logInfo('Vmzarinpalzg_data' . implode('   ', $Vmzarinpalzg_data), 'message');

		$this->_storemellatgadnternalData($method, $Vmzarinpalzg_data, $virtuemart_order_id);

		$new_status = $this->_getPaymentStatus($method, $Vmzarinpalzg_data['payment_status']);
	...
	function _storeVmzarinpalzgnternalData($method, $Vmzarinpalzg_data, $virtuemart_order_id) {
		...
		$post_msg = '';
		foreach ($Vmzarinpalzg_data as $key => $value) {
		...
		$return_context = $Vmzarinpalzg_data['custom'];
		$response_fields['order_number'] = $Vmzarinpalzg_data['invoice'];
	...
	function _getPaymentStatus($method, $Vmzarinpalzg_status) {
		$new_status = '';
		if (strcmp($Vmzarinpalzg_status, 'Completed') == 0) {
			$new_status = $method->status_success;
		} elseif (strcmp($Vmzarinpalzg_status, 'Pending') == 0) {
	...
	function _getPaymentResponseHtml($VmzarinpalzgTable, $payment_name) {

		$html = '<table>' . "\n";
		$html .= $this->getHtmlRow('زرين پال', $payment_name);
		if (!empty($VmzarinpalzgTable)) {
			$html .= $this->getHtmlRow('شماره سفارش', $VmzarinpalzgTable->order_number);
